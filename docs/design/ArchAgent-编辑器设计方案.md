...
首期不包含通用 GLB/OBJ 导入、人物、顶点/边/面级 Mesh 编辑、UV、贴图烘焙、骨骼绑定、动画、IFC 工作流或 Blender 级通用建模。它们是独立能力域，不能反向决定当前建筑建模架构。
...
无法映射为 Pascal 建筑节点的内容必须在进入主场景前被拒绝或转为可解释的建筑参数，不能依赖第二渲染器兜底。
...
| P2 | Zone、Ceiling、Roof、建筑 Item | 空间检测与参数化构件 |
...
2. **P1 空间编辑**：选择、高亮、画墙预览、楼板、门窗、撤销重做、项目持久化与户型图输入。
