...
当前 Agent 工具提供 `get_scene`、墙体/楼板/门/窗的创建和修改工具，以及 `delete_node`：修改或删除前先读取权威场景摘要，随后把校验后的命令交给 `SceneCommandService`。门窗操作以墙体 ID 为前置条件，所有工具都必须经过同一条校验、历史、持久化与 Pascal 投影链路。
...
3. **P1 空间编辑**：画墙预览、Pascal 原生选择高亮、楼板、天花、柱、房间、直梯、围栏、门窗、撤销重做与项目持久化已完成；其余构件鼠标工具和户型图输入仍在后续范围。
4. **P2 Agent 建模**：结构化场景命令、ghost preview、材质规格与参数化构件重生成。
5. **后续建筑能力**：空间检测、屋顶、扩展构件库和建筑格式互操作，均通过建筑命令主链路接入。
