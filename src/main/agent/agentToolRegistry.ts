...
function buildRefactoredAgentChatTools(options: { includeExecBash: boolean }): ChatCompletionTool[] {
  const tools: ChatCompletionTool[] = [
    ...
    { type: "function", function: { name: "generate_3d_asset", description: "生成一个边界干净、可独立摆放的完整低模 GLB，并自动登记到资产库。name 指定目标资产；prompt 可自然包含外观、用途、环境或摆放语境，生成服务会聚焦目标资产本体。不会摆放；返回 library_asset_id，随后可查看预览或用 place_library_assets 实例化。", parameters: generate3dAssetSchema } },
    { type: "function", function: { name: "edit_library_asset", description: "编辑已生成的资产库条目。mode=metadata 只调整名称、描述、分类、标签或目标尺寸，不改 GLB；mode=regenerate 基于原资产描述和 edit_prompt 生成新的可摆放版本，原资产及已放置实例保持不变。edit_prompt 可自然包含外观、用途、环境或摆放语境。", parameters: editLibraryAssetSchema } },
...
const generate3dAssetSchema = { type: "object", properties: { name: { type: "string", description: "目标资产名称，例如“现代三人位沙发”“现代木头床头柜”或“人物坐在单椅上”。" }, source: { type: "string", enum: ["text", "image"] }, prompt: { type: "string", description: "source=text 时必填：中文生成描述。可自然包含外观、材质、用途、环境或摆放语境；服务会以 name 指定的目标资产为生成重点。" }, resource_id: { type: "string", description: "source=image 时必填：已确认且只包含目标资产的资源 ID。" }, target_dimensions_meters: targetDimensionsMetersSchema, category: { type: "string" }, tags: { type: "array", items: { type: "string" }, maxItems: 12 } }, required: ["name", "source"], additionalProperties: false } as const;
const editLibraryAssetSchema = { type: "object", properties: { library_asset_id: { type: "string", description: "search_library_assets 返回的资产库 ID。" }, mode: { type: "string", enum: ["metadata", "regenerate"] }, edit_prompt: { type: "string", description: "mode=regenerate 时必填：描述需要改变的外观、结构、颜色、材质、风格或自然语境。" }, name: { type: "string", description: "调整后的资产名称；mode=regenerate 时同时用于新版本名称。" }, description: { type: "string", description: "mode=metadata 时更新资产描述。" }, target_dimensions_meters: targetDimensionsMetersSchema, category: { type: "string" }, tags: { type: "array", items: { type: "string" }, maxItems: 12 } }, required: ["library_asset_id", "mode"], additionalProperties: false } as const;
