...
function findNearestWall(snapshot: SceneSnapshot, position: [number, number, number]): { wall: SceneWallNode; distance: number; side: "room_interior" | "outside" } | undefined {
  ...
  for (const wall of Object.values(snapshot.nodes)) {
    ...
    nearest = { wall, distance, side: offsetX * leftNormal[0] + offsetZ * leftNormal[1] >= 0 ? "inside" : "outside" };
  }
  return nearest;
}
...
const wallAnchorSchema = { type: "object", description: "以墙体中心点为基准锚定。inside 为从 start→end 的左侧，outside 为右侧。", properties: { element_id: { type: "string", description: "墙体 element_id，或唯一墙体显示名。" }, side: { type: "string", enum: ["inside", "outside"] }, distance: { type: "number", minimum: 0, description: "垂直离墙距离，单位米。" } }, required: ["element_id", "side", "distance"], additionalProperties: false } as const;
