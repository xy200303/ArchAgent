...
export async function generateHunyuanGlb(input: HunyuanImageTo3dInput, outputDir: string): Promise<{ path: string; previewImageUrl?: string; faceCount?: number }> {
  ...
  const generateType = toHunyuanGenerateType(input.generateType ?? "normal");
  ...
  const result = await waitForResult(apiKey, jobId, model);
  ...
  const destination = join(outputDir, `${safeName(input.name)}.glb`);
  ...
  return { path: destination, previewImageUrl: glb.preview_image_url, faceCount };
}
