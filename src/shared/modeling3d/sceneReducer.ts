...
function createAssetCommand(snapshot: SceneSnapshot, input: Extract<SceneCommandInput, { type: "asset.create" }>, createId: (prefix: string) => string): SceneCommandResult {
  ...
  const asset: SceneAssetNode = { id, type: "asset", name: input.name?.trim() || `参考模型 ${countNodes(snapshot, "asset") + 1}`, parentId: input.parentId, format: input.format, sourcePath: input.sourcePath, position: input.position ?? [0, 0, 0], rotation: input.rotation ?? [0, 0, 0], scale: input.scale ?? [1, 1, 1], ...(input.targetDimensions ? { targetDimensions: input.targetDimensions } : {}), ...(input.footprint ? { footprint: input.footprint } : {}) };
  if (![...asset.position, ...asset.rotation, ...asset.scale, ...(asset.targetDimensions ?? [])].every(Number.isFinite) || asset.scale.some((value) => value <= 0) || (asset.targetDimensions && asset.targetDimensions.some((value) => value <= 0)) || (asset.footprint && (!asset.footprint.every(Number.isFinite) || asset.footprint.some((value) => value <= 0)))) return failure("invalid_command", "参考模型的变换、目标尺寸或占地参数无效。");
  const command: SceneCommand = { type: "asset.create", id, parentId: asset.parentId, name: asset.name, format: asset.format, sourcePath: asset.sourcePath, position: asset.position, rotation: asset.rotation, scale: asset.scale, ...(asset.targetDimensions ? { targetDimensions: asset.targetDimensions } : {}), ...(asset.footprint ? { footprint: asset.footprint } : {}) };
