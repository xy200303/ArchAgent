...
export function AssetInspector({ asset, onUpdate, onDelete, onClose }: {
  ...
  function setVectorValue(field: "position" | "rotation" | "scale", index: number, value: string): void {
    setDraft((current) => {
...
function VectorFields({ title, value, axes, min, onChange }: { title: string; value: [string, string, string]; axes: [string, string, string]; min?: string; onChange: (index: number, value: string) => void }): JSX.Element {
  return <fieldset className="asset-vector-fields"><legend>{title}</legend><div className="wall-property-grid">{value.map((item, index) => <label key={axes[index]}><span>{axes[index]}</span><input type="number" min={min} step="0.1" value={item} onChange={(event) => onChange(index, event.target.value)} required /></label>)}</div></fieldset>;
}

function createDraft(asset: SceneAssetNode): AssetDraft {
  return { name: asset.name, position: asset.position.map(String) as AssetDraft["position"], rotation: asset.rotation.map((value) => toDegrees(String(value))) as AssetDraft["rotation"], scale: asset.scale.map(String) as AssetDraft["scale"] };
}
...
function toAssetUpdate(draft: AssetDraft): Omit<Extract<SceneCommandInput, { type: "asset.update" }>, "type" | "id"> | undefined {
  ...
  if (!position || !rotationDegrees || !scale || scale.some((value) => value <= 0) || !draft.name.trim()) return undefined;
  return { name: draft.name.trim(), position, rotation: rotationDegrees.map(toRadians) as [number, number, number], scale };
}
